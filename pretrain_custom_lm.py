...
"""
Pr-treinamento de um pequeno modelo de linguagem
=================================================

Fluxo:
  1. Usa o texto do README (artigo/resumo) como corpus inicial.
  2. Treina um tokenizador SentencePiece (BPE) se ainda no existir.
  3. Constri um Dataset de LM (inputs + labels deslocados).
...
class LMDataset(Dataset):
    ...
    def __init__(self, token_ids: List[int], block_size: int) -> None:
        ...
        # Trunca para mltiplo de block_size
...
def build_token_stream(tokenizer: CustomSPTokenizer) -> Tuple[List[int], List[int]]:
    """
    Constri streams de tokens para treino e validao a partir do corpus principal.
    Usa diviso simples train/val em nvel de documento.
    """
    texts = load_main_corpus()
    if len(texts) == 1:
        train_texts = texts
...
def train_lm(config: TrainLMConfig) -> EpistemicLanguageModel:
    ...
    # Suporte simples a mltiplas GPUs via DataParallel
